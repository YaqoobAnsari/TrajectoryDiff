...
class KrigingBaseline:
    ...
    def __call__(
        ...
        # Simple Kriging with sample mean: work with residuals so predictions
        # revert to the data mean (not zero) at unobserved locations.
        mu = np.mean(values)
        residuals = values - mu

        # Build covariance matrix (vectorized pairwise distances)
        ...
        # Precompute alpha = C_inv @ residuals (n_samples,)
        # Key optimization: result[q] = mu + c0[q] @ C_inv @ residuals
        # This avoids the expensive (chunk, n) @ (n, n) matmul per chunk,
        # replacing it with (chunk, n) @ (n,) matvec — ~n times faster.
        alpha = C_inv @ residuals  # (n_samples,)

        # For uncertainty, we also need C_inv directly
        need_full_weights = return_uncertainty

        # Vectorized interpolation: process in row chunks to limit memory
        yy, xx = np.meshgrid(np.arange(H), np.arange(W), indexing='ij')
        query_x = xx.ravel()  # (H*W,)
        query_y = yy.ravel()  # (H*W,)

        n_query = H * W
        # Use larger chunks for speed (4 rows at a time, ~4*W*n*8 bytes)
        chunk_size = W * 4
        result_flat = np.zeros(n_query, dtype=np.float64)
        uncertainty_flat = np.zeros(n_query, dtype=np.float64) if return_uncertainty else None

        for row_start in range(0, n_query, chunk_size):
            row_end = min(row_start + chunk_size, n_query)
            qx = query_x[row_start:row_end]  # (chunk,)
            qy = query_y[row_start:row_end]  # (chunk,)

            # Distances: (chunk, n_samples)
            dx = qx[:, None] - points[:, 0][None, :]
            dy = qy[:, None] - points[:, 1][None, :]
            dist = np.sqrt(dx**2 + dy**2)

            # Covariance vectors: (chunk, n_samples)
            c0 = self.sill + self.nugget - self._spherical_variogram(dist)

            # Fast prediction via precomputed alpha: mu + (chunk, n) @ (n,) = (chunk,)
            # At observed locations, c0 reconstructs residuals; far away, c0 -> 0
            # so prediction reverts to mu (the sample mean), not zero.
            result_flat[row_start:row_end] = mu + c0 @ alpha

            if need_full_weights:
                weights = c0 @ C_inv  # (chunk, n) @ (n, n) — only when needed
                uncertainty_flat[row_start:row_end] = (
                    self.sill + self.nugget - np.sum(c0 * weights, axis=1)
                )

        result = result_flat.reshape(H, W)

        if return_uncertainty:
            uncertainty = np.sqrt(np.maximum(uncertainty_flat.reshape(H, W), 0))
            return result.astype(np.float32), uncertainty.astype(np.float32)
        return result.astype(np.float32)
