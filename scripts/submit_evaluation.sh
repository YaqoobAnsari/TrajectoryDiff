...
if [ -z "${CHECKPOINT:-}" ]; then
    ...
    CKPT_FOUND=""
    # Search in reverse chronological order (newest experiment dir first)
    for exp_dir in $(ls -1rd experiments/${EXPERIMENT}/*/checkpoints experiments/${EXPERIMENT}/checkpoints 2>/dev/null); do
        if [ ! -d "$exp_dir" ]; then
            continue
        fi
