...
def evaluate_model(
    ...
    all_psnr_observed = []
    all_psnr_unobserved = []
    # Per-region dBm metrics (free-space / building breakdown)
    all_rmse_free_space = []
    all_mae_free_space = []
    all_rmse_building = []
    all_rmse_free_obs = []
    ...
    for batch in tqdm(dataloader, desc="Evaluating"):
        ...
        # Per-region dBm metrics: free-space vs building breakdown
        if 'building_map' in batch:
            building_map = batch['building_map']
            traj_mask = batch.get('trajectory_mask')
            for b in range(samples_dbm.shape[0]):
                bmap = building_map[b, 0].cpu().numpy()
                pred_np = samples_dbm[b, 0].cpu().numpy()
                gt_np = gt_dbm[b, 0].cpu().numpy()

                # Convention: building_map > 0.0 = free space (for [-1,1] normalized data)
                free_space = bmap > 0.0
                building = ~free_space

                # Free-space RMSE/MAE
                if free_space.any():
                    diff_fs = pred_np[free_space] - gt_np[free_space]
                    all_rmse_free_space.append(float(np.sqrt(np.mean(diff_fs ** 2))))
                    all_mae_free_space.append(float(np.mean(np.abs(diff_fs))))
                ...
                # Building RMSE
                if building.any():
                    diff_b = pred_np[building] - gt_np[building]
                    all_rmse_building.append(float(np.sqrt(np.mean(diff_b ** 2))))

                # Free-space observed / unobserved (requires trajectory_mask)
                if traj_mask is not None:
                    tmask = traj_mask[b, 0].cpu().numpy() > 0.5
                    free_obs = free_space & tmask
                    free_unobs = free_space & ~tmask

                    if free_obs.any():
                        diff_fo = pred_np[free_obs] - gt_np[free_obs]
                        all_rmse_free_obs.append(float(np.sqrt(np.mean(diff_fo ** 2))))
                    if free_unobs.any():
                        diff_fu = pred_np[free_unobs] - gt_np[free_unobs]
                        all_rmse_free_unobs.append(float(np.sqrt(np.mean(diff_fu ** 2))))
    ...
    # Per-region dBm metrics (free-space / building breakdown)
    if all_rmse_free_space:
        metrics['rmse_free_space_dbm'] = float(np.mean(all_rmse_free_space))
        metrics['rmse_free_space_dbm_std'] = float(np.std(all_rmse_free_space))
        metrics['mae_free_space_dbm'] = float(np.mean(all_mae_free_space))
        metrics['mae_free_space_dbm_std'] = float(np.std(all_mae_free_space))
        metrics['per_sample_rmse_free_space_dbm'] = all_rmse_free_space
    if all_rmse_building:
        metrics['rmse_building_dbm'] = float(np.mean(all_rmse_building))
        metrics['rmse_building_dbm_std'] = float(np.std(all_rmse_building))
    if all_rmse_free_obs:
        metrics['rmse_free_obs_dbm'] = float(np.mean(all_rmse_free_obs))
        metrics['rmse_free_obs_dbm_std'] = float(np.std(all_rmse_free_obs))
    if all_rmse_free_unobs:
        metrics['rmse_free_unobs_dbm'] = float(np.mean(all_rmse_free_unobs))
        metrics['rmse_free_unobs_dbm_std'] = float(np.std(all_rmse_free_unobs))
        metrics['per_sample_rmse_free_unobs_dbm'] = all_rmse_free_unobs
    if all_ssim_free_space:
        metrics['ssim_free_space'] = float(np.nanmean(all_ssim_free_space))
        metrics['ssim_free_space_std'] = float(np.nanstd(all_ssim_free_space))
...
def main(cfg: DictConfig):
    ...
    # Per-region metrics (free-space / building breakdown)
    if 'rmse_free_space_dbm' in metrics:
        print(f"\nPer-Region Metrics (dBm scale):")
        print(f"  Free-Space RMSE:     {metrics['rmse_free_space_dbm']:.2f} +/- {metrics['rmse_free_space_dbm_std']:.2f} dBm")
        print(f"  Free-Space MAE:      {metrics['mae_free_space_dbm']:.2f} +/- {metrics['mae_free_space_dbm_std']:.2f} dBm")
        if 'rmse_building_dbm' in metrics:
            print(f"  Building RMSE:       {metrics['rmse_building_dbm']:.2f} +/- {metrics['rmse_building_dbm_std']:.2f} dBm")
        if 'rmse_free_obs_dbm' in metrics:
            print(f"  Free-Space Obs RMSE: {metrics['rmse_free_obs_dbm']:.2f} +/- {metrics['rmse_free_obs_dbm_std']:.2f} dBm")
        if 'rmse_free_unobs_dbm' in metrics:
            print(f"  Free-Space Unobs RMSE: {metrics['rmse_free_unobs_dbm']:.2f} +/- {metrics['rmse_free_unobs_dbm_std']:.2f} dBm  *** KEY METRIC ***")
        if 'ssim_free_space' in metrics:
            print(f"  Free-Space SSIM:     {metrics['ssim_free_space']:.4f} +/- {metrics['ssim_free_space_std']:.4f}")

    if 'ssim_observed' in metrics:
        print(f"\nObserved/Unobserved SSIM & PSNR:")
