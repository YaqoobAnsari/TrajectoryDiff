...
#SBATCH --output=experiments/logs/ddim_ablation_%A.out
#SBATCH --error=experiments/logs/ddim_ablation_%A.err
#
# DDIM Steps Ablation: Evaluate trajectory_full with T={10, 25, 50, 100}
# Shows speed-quality tradeoff for DDIM sampling.
#
# Usage:
#   sbatch scripts/submit_ddim_ablation.sh

set -euo pipefail

echo "=============================================="
echo "DDIM Steps Ablation (SLURM)"
echo "=============================================="
...
for ddim_steps in ddim_steps_list:
    ...
    # Replace the DDIM sampler with new step count
...
with open(out_dir / 'ddim_ablation.json', 'w') as f:
    json.dump(results, f, indent=2)

# Summary table
print(f'\n{\"=\"*60}')
print('DDIM Steps Ablation — Summary')
print(f'{\"=\"*60}')
print(f'{\"Steps\":<8} {\"Free-unobs RMSE\":>18} {\"SSIM (free)\":>12} {\"ms/sample\":>10}')
print('-' * 52)
for key in sorted(results.keys(), key=lambda x: results[x]['ddim_steps']):
    r = results[key]
    print(f'T={r[\"ddim_steps\"]:<5} {r[\"rmse_free_unobs_dbm\"]:>14.2f} dBm {r[\"ssim_free_space\"]:>12.4f} {r[\"time_per_sample_ms\"]:>10.0f}')

print(f'\nResults saved to: {out_dir / \"ddim_ablation.json\"}')
"

echo ""
echo "=============================================="
echo "DDIM ablation complete at $(date)"
echo "=============================================="
